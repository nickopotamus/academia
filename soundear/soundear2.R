...
# definitions
input = "a2_all_nights.csv"                     # input data file
...
# data input
dat = read.csv(input)
bad = which(dat[,"ambient"] < xlim[1] | dat[,"ambient"] > xlim[2])
dat = dat[-bad,]
int0 = dat[which(dat[,"int"]==1),"ambient"]
int1 = dat[which(dat[,"int"]==2),"ambient"]

# Gaussian fitting
cat("int=1"); fit0 = normalmixEM(int0, epsilon=epsilon, lambda=c(0.4,0.2,0.4), mu=c(51,58,60), sigma=c(2.2,0.6,3.7))
cat("int=2"); fit1 = normalmixEM(int1, epsilon=epsilon, lambda=c(0.5,0.5), mu=c(51,57), sigma=c(1.7,4.1))
