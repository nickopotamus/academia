...
a2 <- read.csv ("a2_nightsonly_sec.csv")
pre <- subset(a2, a2$int==0)
post <- subset(a2, a2$int==1)


mdl.C.pre  <- normalmixEM(pre$dBCfast, k=2)
mdl.C.post <- normalmixEM(post$dBCfast, k=2)
...
Iter <- 0
bs.A.pre <- tsbootstrap(pre$dBAslow, nb=1000, statistic=mix.stats)
bs.A.post <- tsbootstrap(post$dBAslow, nb=1000, statistic=mix.stats)

mdl.A.pre  <- norMixEM(pre$dBAslow, 2)
mdl.A.post <- norMixEM(post$dBAslow, 2)
